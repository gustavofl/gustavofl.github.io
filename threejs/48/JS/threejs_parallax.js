...
window.addEventListener('load', async () => {
  ...
  function setScale(separation) {
    ...
    if (mesh_note) {
      ...
      if (mesh_note.visible) {
        document.getElementById("scale-message-div").textContent = Math.round(goalScale * 100) + "%";
      }
    }
  }
  ...
  async function render(timestamp, frame) {
    if (frame) {
      ...
      if (mesh_note.visible && isTwoFingering) {
        let scaleMessageDiv = document.getElementById("scale-message-div");
        scaleMessageDiv.style.display = "block";
        scaleMessageDiv.classList.add('visivel');
        scaleMessageDiv.classList.remove('escondendo');
      } else {
        let scaleMessageDiv = document.getElementById("scale-message-div");
        scaleMessageDiv.classList.remove('visivel');
        scaleMessageDiv.classList.add('escondendo');

        scaleMessageDiv.addEventListener('transitionend', function handleTransitionEnd() {
          minhaDiv.style.display = 'none';
          minhaDiv.removeEventListener('transitionend', handleTransitionEnd);
        });
      }

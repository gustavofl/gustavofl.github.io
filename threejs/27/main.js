  ...
  function render(timestamp, frame) {
    if (frame) {
      ...
      if (! overlay_listeners_created) {
        const div_overlay = document.getElementById("div_overlay");
    
        div_overlay.addEventListener("touchstart", (event) => {
          lastX = event.touches[0].clientX;
        });
        
        div_overlay.addEventListener("touchmove", (event) => {
          const currentX = event.touches[0].clientX;
          if (lastX !== null) {
            const deltaX = currentX - lastX;
            rotacao_note += deltaX * 0.5;
            lastX = currentX;
          }
        });
        
        div_overlay.addEventListener("touchend", () => {
          lastX = null;
        });
      ...
      if (hitTestSource) {
        ...
        if (hitTestResults.length) {
          ...
          document.getElementById("rotation-prompt").style.display = "block";
      } else {
          if (planeFound) {
            planeFound = false;
          }

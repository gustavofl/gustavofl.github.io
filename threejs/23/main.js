...
window.addEventListener('load', async () => {
  ...
  function render(timestamp, frame) {
    if (frame) {
        ...
        session.requestReferenceSpace("viewer").then(function (referenceSpace) {
          session
            ...
            .then(function (source) {
              Bugfender.log("atribuindo hitTestSource")
              hitTestSource = source;
            });
        });

        session.addEventListener("end", function () {
          Bugfender.log("removendo atribuição hitTestSource")
      ...
      if (hitTestSource) {
        // Bugfender.log(hitTestSource);

        const hitTestResults = frame.getHitTestResults(hitTestSource);

        Bugfender.log(hitTestResults.length);
        ...
        if (hitTestResults.length) {
          ...
          if (! possui_instancia) {
            ...
            reticleParent.matrix.fromArray(hit.getPose(referenceSpace).transform.matrix);

...
window.addEventListener('load', async () => {
  ...
  async function render(timestamp, frame) {
    if (frame) {
      ...
      try {
        if (rotacionando_direita || rotacionando_esquerda) {
          let rotacao = rotacionando_direita ? ROTATION_SPEED : -ROTATION_SPEED;

          if (mesh_note) {
            mesh_note.rotateY(rotacao);
          }

          if (hitPlane) {
            hitPlane.rotateY(rotacao);
          }
        }
      } catch (error) {

...
window.addEventListener('load', async () => {
  ...
  function render(timestamp, frame) {
    ...
    if (frame) {
      ...
      // o iPhone demora pra reconhecer o frame, demorando para remover o notebook. Ver outra forma de remover o notebook.
      if (rodando_ar === false) {
        remove_note();
        rodando_ar = true;
      }
      ...
      if (hitTestSource) {
        ...
        if (hitTestResults.length) {
          ...
          // Bugfender.log(hit.getPose(referenceSpace).transform.matrix)

          // Extrai apenas a translação e escala da matriz do hit test
          const hitMatrix = hit.getPose(referenceSpace).transform.matrix;
          // Decompor a matriz em posição, quaternion e escala
          const tempMatrix = new THREE.Matrix4().fromArray(hitMatrix);
